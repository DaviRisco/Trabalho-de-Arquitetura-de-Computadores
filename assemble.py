...
instructions = ['add', 'sub', 'goto', 'mov', 'jz', 'halt', 'lst', 'rst', 'rem','wb', 'ww']
...
def encode_instruction(inst, ops):
   if inst == 'add' or inst == 'sub' or inst == 'mov' or inst == 'jz' or inst == 'lst' or inst == 'rst' or inst == 'rem':
...
def resolve_names():
   ...
   for line in lines_bin:
      for i in range(0, len(line)):
         if is_name(line[i]):
            if line[i-1] == instruction_set['add'] or line[i-1] == instruction_set['sub'] or line[i-1] == instruction_set['mov'] or line[i-1] == instruction_set['lst'] or line[i-1] == instruction_set['rst'] or line[i-1] == instruction_set['rem']:
